...
cache_dir = f"/tmp/cache_saperlipopet_qmd_{snakemake.params['ann_method']}_" + date_time_launch
...
logger.debug("REF_PATH = " + REF_PATH)
...
ref = anndata.io.read_h5ad(REF_PATH)
...
ident_types = query.obs["ident"].unique()
logger.debug(cell_types)
logger.debug(ident_types)
# Create a dictionary mapping cell types to colors
colors = ["#f464e6", "#004bbd", "#00bd00", "#d52d00", "#f8ae14", "#f2f864", "#9df3f1"]
pred_types_colors = dict(zip(cell_types, colors[:len(cell_types)]))
ident_types_colors = dict(zip(ident_types, colors[:len(ident_types)]))

cell_type_colors = ident_types_colors | pred_types_colors
logger.debug(cell_type_colors)
...
## Row
```{python}
#| title: Final predicted labels and true cell types
sc.pl.umap(
    query,
    color=["final_prediction", "ident"],
    title=["Predicted labels after metaclassification", "True cell types"],
    size=10,
...
y_true = query.obs["ident"]
y_pred = query.obs["final_prediction"]

# confusion matrix
labels = sorted(set(query.obs["final_prediction"]).union(query.obs["ident"]))
...
cm_perc_row = np.divide(cm, row_sums, where=row_sums!=0) * 100
cm_perc_col = np.divide(cm, col_sums, where=col_sums!=0) * 100

# annotations
annot = np.empty_like(cm).astype(str)
for i in range(cm.shape[0]):
    for j in range(cm.shape[1]):
        row_percent = cm_perc_row[i, j]
        col_percent = cm_perc_col[i, j]
        count = cm[i, j]
        annot[i, j] = f"{row_percent:.1f}%\n({count})\n({col_percent:.1f}%)"
...
plt.title("Confusion matrix of the final prediction", fontsize=14)
...
summary = (
    query.obs.groupby(["orig.ident", "ident"]).size()
...
```{python}
#| title: Models agreement on final prediction and cells correctly annotated or not
...
query.obs["final_pred_correct"] = query.obs["final_prediction"].astype(str) == query.obs["ident"].astype(str)
sc.pl.umap(
    query,
    color=["pred_label_proportion", "final_pred_correct"],
    size=10,
    color_map="RdYlGn",
    palette=true_false_colors,
    frameon=False
)
```
...
sc.pl.umap(
    query,
    color=["proportion_difference", "final_pred_correct"],
    size=10,
    color_map="RdYlGn",
    palette=true_false_colors,
    frameon=False
...
for model in models:
    ...
    sc.pl.umap(
        query,
        color=[f"predictions_{model}", "ident"],
    ...
    sc.pl.umap(
        query,
        color=[f"binary_mapqc_score_{model}", "ident"],
        title=f"Binary MapQC scores for the integration of {model}",
        size=30,
        palette= binary_mapqc_colors|cell_type_colors,
        frameon=False
    )
    display(Markdown(""))
    ...
    sc.pl.umap(
        query,
        color=[f"mapqc_score_{model}", "ident"],
